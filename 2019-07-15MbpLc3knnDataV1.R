...
# Normalize (using max/min scaling) matching criteria (signup month PV, LOI flag)
query_results$pv_normal <- (query_results$mth_pv_amt - min(query_results$mth_pv_amt))/(range(query_results$mth_pv_amt)[2]-range(query_results$mth_pv_amt)[1])
query_results$loi_normal <- (query_results$mth_loi_flg - min(query_results$mth_loi_flg))/(range(query_results$mth_loi_flg)[2]-range(query_results$mth_loi_flg)[1])
query_results$spon_normal <- (query_results$mth_spon_all_cnt - min(query_results$mth_spon_all_cnt))/(range(query_results$mth_spon_all_cnt)[2]-range(query_results$mth_spon_all_cnt)[1])
...
#############################################################################################################################################
# STEP THREE: PAIRING #
# The function called 'pairup' is in the get_LC_data.R file
...
matched_ind <- pairup(LC_dat[c('pv_normal','loi_normal','spon_normal')], nonLC_dat[c('pv_normal','loi_normal','spon_normal')]) # Returns indices of non-LC people who are matched
#names(matched_ind) <- 'nonLC_matched_ind' #rename column name for convenience
names(matched_ind) <- NULL

# How well does the pairing do?
matched_pv <- cbind(LC_dat[,c(23)],nonLC_dat[matched_ind,c(23)]) #make a data set of LC PV and pair
summary(matched_pv[,1]-matched_pv[,2])
#############################################################################################################################################
...
nonLC_dat$cntrl_group_flg[seq.int(nrow(nonLC_dat)) %in% as.list(matched_ind)] <- 1 # only set the control group flag if we matched them (not everyone in the nonLC group will be matched)

data_final <- rbind(nonLC_dat, LC_dat) #, midLC)
data_final$pv_normal <- NULL; data_final$loi_normal <- NULL; data_final$spon_normal <- NULL
#############################################################################################################################################
...
bqr_upload_data(projectId = "nu-skin-corp", datasetId = "REPORTING",tableId = "SUMMARY_MONTH_ONBOARDING_3KnnMatch",
...
#update_2ndmonth_pv(strt_date = "'2019-02-01'", end_date = "'2019-05-01'")
